...
base_run_num=31 #used for filenames for saving
...
then
python plot_mbmf.py --std_on_mlp_policy=$std_on_mlp_policy --which_agent=$which_agent --run_nums 31 32 33 --seeds ${seeds[0]} ${seeds[1]} ${seeds[2]}
fi

if [ $how_many_seeds -eq 2 ]
then
python plot_mbmf.py --std_on_mlp_policy=$std_on_mlp_policy --which_agent=$which_agent --run_nums 31 32 --seeds ${seeds[0]} ${seeds[1]}
fi

if [ $how_many_seeds -eq 1 ]
then
python plot_mbmf.py --std_on_mlp_policy=$std_on_mlp_policy --which_agent=$which_agent --run_nums 31 --seeds ${seeds[0]}
fi