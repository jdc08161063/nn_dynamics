...
def make_trajectory(shape, starting_state_NN, x_index, y_index, which_agent):
    ...
    if(shape=="left_turn"):
        if(which_agent==1):
            ...
            my_list.append(np.array([curr_x+4, curr_y+5]))
    ...
    if(shape=="right_turn"):
        if(which_agent==1):
            ...
            my_list.append(np.array([curr_x, curr_y]))
            my_list.append(np.array([curr_x, curr_y+1]))
            my_list.append(np.array([curr_x+0.5, curr_y+2]))
            my_list.append(np.array([curr_x+0.75, curr_y+3]))
            my_list.append(np.array([curr_x+1, curr_y+4]))
            my_list.append(np.array([curr_x+2, curr_y+4]))
            my_list.append(np.array([curr_x+3, curr_y+4]))
            my_list.append(np.array([curr_x+4, curr_y+4]))
            my_list.append(np.array([curr_x+5, curr_y+4]))
...
def get_trajfollow_params(which_agent, desired_traj_type):
    ...
    if(which_agent==2):
        if(desired_traj_type=="right_turn"):
            ...
            forward_encouragement_factor= 80
            heading_penalty_factor= 0.2
        if(desired_traj_type=="left_turn"):
            horiz_penalty_factor= 0.5
            forward_encouragement_factor= 900
