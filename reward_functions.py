...
######################################################################################################################
    ...
    def swimmer_follow_traj(self, pt, prev_pt, scores, min_perp_dist, curr_forward, prev_forward, 
        ...
        backward_discouragement= 7.5
        scores[np.logical_and(moved_to_next==0, curr_forward<=prev_forward)] += backward_discouragement
        scores[curr_forward>prev_forward] -= self.forward_encouragement_factor*(curr_forward - prev_forward)[curr_forward>prev_forward]
